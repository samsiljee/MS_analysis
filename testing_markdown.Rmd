...
unnormalised_boxplot <- unnormalised %>%
  ggplot(aes(x = Run, y = log2(Intensity), col = Condition)) +
  geom_boxplot() +
  theme_bw() +
...
equalised_medians_boxplot <- equalised_medians %>%
  ggplot(aes(x = originalRUN, y = log2(INTENSITY), col = GROUP)) +
  geom_boxplot() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90)) +
