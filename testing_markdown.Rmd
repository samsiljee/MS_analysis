...
# STRING analysis
```{r STRING, echo = FALSE}
# define background first
bg_prots <- STRINGdb$map(as.data.frame(input), "ProteinName", removeUnmappedRows = TRUE) %>%
  .$STRING_id %>%
  unique()
...
string_db <- STRINGdb$new( version="11.5",
                           ...
                           input_directory="",
                           protocol = "http",
                           backgroundV = unique(input$ProteinName))

STRING_dataset <- MSstats.results %>%
    filter(Label == "HEK vs SeasonB") %>%
...
# Run enrichment
#set background
bg_prots <- string_db$map(as.data.frame(input), "ProteinName", removeUnmappedRows = TRUE) %>%
  .$STRING_id %>%
  unique()
string_db$set_background(bg_prots)
