...
basalCellGenes <- read.csv("~/Coding/MS_analysis/tracked_files/basalCellTranscriptome.csv") %>%
  ...
  unique() %>%
  sort()
#Adding comparison for cila related genes. taken from http://www.syscilia.org/goldstandard.shtml and edited to remove all unrelated columns
ciliatedCellGenes <- read.table("~/Coding/MS_analysis/tracked_files/scgs.v1.tsv", sep = "\t") %>%
  .$V1 %>% 
  unique() %>%
  sort()

#creating a list of all genes from the KEGG pathways. I'm sure there's a much better way to do this, but I don't know how exactly at this point.
...
print("Proportion of basal cell markers found - total list (303)")
length(ciliatedMarkersPresent)/length(ciliatedCellGenes)

print("Proportion of basal cell markers found - monte carlo (58)")
hits <- replicate(n = 10000, expr = {(evidence %>%
    filter(firstGeneName %in% sample(ciliatedCellGenes, 58, replace = FALSE)) %>%
    .$firstProtName %>%
    length())/58})
mean(hits)
```
